...
class Stats(object):
    ...
    @staticmethod
    def _get_historical_freeze(report_id, frozen_image):
...
class EELandsat(object):
    ...
    def find_mapid_from_sensor(self, bound=None):
        ...
        if image:
            raw_mapid = _get_raw_mapid(image.getMapId({
               'bands': ','.join(map_image_bands),
               'gain': self.PREVIEW_GAIN
            }))
        else:
            raw_mapid = _get_raw_mapid(None)

        return raw_mapid

    def get_thumbs(self, tiles):
        results = {}
        ...
        for tile in tiles:
            ...
            # region = feature.bounds(self.MAX_ERROR_METERS)
            ...
            for i in range(len(collections)):
                ...
                for j in range(len(images)):
                    ...
                    thumbs_info.append({'thumb': result['thumbid'],
                                        'token': result['token'],
                                        'date': date,
                                        'map_image': map_image,
                                        'tile': tile.replace("/","_")})
    ...
    def find_collection_tile(sensor, tile, start_date, end_date=None):
        ...
        if end_date:
            collection = ee.ImageCollection(sensor).filterMetadata('WRS_PATH', 'equals', int(wrs_path))\
                .filterMetadata('WRS_ROW', 'equals', int(wrs_row)).filterDate(start_date, end_date)
        else:
            collection = ee.ImageCollection(sensor).filterMetadata('WRS_PATH', 'equals', int(wrs_path))\
                .filterMetadata('WRS_ROW', 'equals', int(wrs_row)).filterMetadata('DATE_ACQUIRED', 'equals', start_date)
    ...
    def find_image_tile_from_all_sensors(self, tile, cloud_cover=30):        
        ...
        collection_l8 = ee.ImageCollection(EELandsat.LANDSAT8).filterMetadata('WRS_PATH', 'equals', int(wrs_path))\
            .filterMetadata('WRS_ROW', 'equals', int(wrs_row)).filterDate(self.start, self.end)\
            .filterMetadata('CLOUD_COVER', 'less_than', int(cloud_cover))
        ...
        collection_l7 = ee.ImageCollection(EELandsat.LANDSAT7).filterMetadata('WRS_PATH', 'equals', int(wrs_path))\
            .filterMetadata('WRS_ROW', 'equals', int(wrs_row)).filterDate(self.start, self.end)\
            .filterMetadata('CLOUD_COVER', 'less_than', int(cloud_cover))
    ...
    def find_image_tile_from_L5(self, tile, cloud_cover=30):
        ...
        collection = ee.ImageCollection(EELandsat.LANDSAT5).filterMetadata('WRS_PATH', 'equals', int(wrs_path))\
            .filterMetadata('WRS_ROW', 'equals', int(wrs_row)).filterDate(self.start, self.end)\
            .filterMetadata('CLOUD_COVER', 'less_than', int(cloud_cover))
    ...
    def find_map_collection(self, bounds=None):
        ...
        return collection_size != 0 if collection else None
...
def create_tile_baseline(start_date, end_date, cell_name):  
    ...
    for i in range(len(tile_image_picker)):
        if len(tile_image_picker[i]) == 1:
            logging.info(tile_image_picker[i][0])
            image_picker = tile_image_picker[i][0]
            sensor     = image_picker['sensor']
            tile_name  = image_picker['cell']
        
            day  = str(image_picker['day']).zfill(2)
            ...
            #logging.info(collection.getInfo())
            ...
            gvs.append(landsat_data_stack['gv'])
            npvs.append(landsat_data_stack['npv'])
            soils.append(landsat_data_stack['soil'])
            clouds.append(landsat_data_stack['cloud'])
            shades.append(landsat_data_stack['shade'])
            shades_median.append(landsat_data_stack['shade_median'])
            cloud_regions.append(landsat_data_stack['cloud_region'])
    ...
    if len(baselines) > 0:                            
        ...
        image_sma = ee.ImageCollection(smas).mosaic().clip(polygon) #TODO: remover e ajustar as dependencias
...
def get_landsat_data_stack(image, sensor, start_date, end_date=None, cell=None, tile_name=None):
...
def create_tile_timeseries(start_date, end_date, cell_name):
    ...
    if len(ndfis) > 0:                        
        ...
        image_ndfi_rgb     = ee.ImageCollection(ndfi_rgbs).mosaic().clip(polygon)
        image_gv           = ee.ImageCollection(gvs).mosaic().clip(polygon)
        image_npv          = ee.ImageCollection(npvs).mosaic().clip(polygon)
        image_soil         = ee.ImageCollection(soils).mosaic().clip(polygon)
        image_cloud        = ee.ImageCollection(clouds).mosaic().clip(polygon)
...
#        image_ndfi = image_ndfi.where(image_last_map.eq(3), 255)
        ...
        feature_ndfi_rgb = ee.Image(image_ndfi_rgb).getMapId({
                              'bands': 'vis-red,vis-green,vis-blue',
                              'gain': 1,
                              'bias': 0.0,
                              'gamma': 1.6                            
        ...
        resutls.append({'id':          feature_ndfi_rgb['mapid'],
                        'token':       feature_ndfi_rgb['token'],
                        'type':        'xyz',
                        'visibility':  True,
                        'description': 'NDFI',
                        'url': 'https://earthengine.googleapis.com/map/'+feature_ndfi_rgb['mapid']+'/{Z}/{X}/{Y}?token='+feature_ndfi_rgb['token']
                        })
