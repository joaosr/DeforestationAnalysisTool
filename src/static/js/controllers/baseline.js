...
var Period = Backbone.View.extend({
    ...
    initialize: function() {
       _.bindAll(this, 'update_range_date', 'render', 'visibility_change');
       var self = this;
...
var EditorBaselineImagePicker = Backbone.View.extend({
    ...
    send_image_picker: function(e) {
        ...
        var message = $.ajax({
                              url: "/imagepicker_baseline/",
                              type: 'POST',
                              data: {thumbs_baseline: thumbs_baseline.join(), date_start: this.date_start,  date_end: this.date_end, cell_name: this.cell_name},
                              dataType: 'json',
                              async: true,
                              success: function(d) {			                	          
	        	                  console.log(d.result);
    ...
    genarete_baseline: function(e) {
    	...
    	this.baseline_layers.url = "/baseline_on_cell/"+date_start+"/"+date_end+"/"+this.cell_name+"/"
...
var Baseline = Backbone.View.extend({
    ...
    initialize: function(){
        _.bindAll(this, 'callback', 'hide_report_tool_bar', 'show_report_tool_bar', 'hide_image_picker', 'show_image_picker', 'visibility_change', 'setting_baseline_popup', 'show_imagepicker_search', 'set_selected');
        this.callerView = this.options.callerView;
        this.report     = this.options.report;
        this.map        = this.options.mapview; 
        //this.setting_report_data();
        this.report_tool_bar = new Period({el: this.$("#date_range"), report: this.report, url_send: '/baseline_report/', callerView: this});
    ...
    setting_baseline_popup: function(popup, grid){
    	...
    	if(this.selected && grid.model.get('z') == '2'){
    		...
    		if(baseline_lay){
    			setting_baseline.find('#rebuild_baseline').click(function(e) {
        			if(e)e.preventDefault();
        			self.show_imagepicker_search(grid, e);
    			});
    			
    			setting_baseline.find('#load_baseline').click(function(e) {
        			if(e)e.preventDefault();
        			self.genarete_baseline(e, baseline_lay.start, baseline_lay.end, cell_name);
    			});
    			...
    			setting_baseline.find('#build_baseline').click(function(e) {
        			if(e)e.preventDefault();
        			self.show_imagepicker_search(grid, e);
    			});
	...
	setting_baseline_layers: function(cell) {
    	...
    	if(item){
    		...
    		//this.baselines = new LayerCollection(item.baseline_response);
    		var layer_names = [];
    		...
    		for(var i = 0; i < layer_names.length; i++){
    			...
    			console.log(name);
    			if(name === 'RGB'){
    				map_one_layer_status   = map_one_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_two_layer_status   = map_two_layer_status + '"' + name + '","' + 'true' + '",';
    	    		map_three_layer_status = map_three_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_four_layer_status  = map_four_layer_status + '"' + name + '","' + 'false' + '",';
    			}
    			else if(name === 'NDFI'){
    				map_one_layer_status   = map_one_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_two_layer_status   = map_two_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_three_layer_status = map_three_layer_status + '"' + name + '","' + 'true' + '",';
    	    		map_four_layer_status  = map_four_layer_status + '"' + name + '","' + 'false' + '",';
    			}
    			else if(name === 'SMA'){
    				map_one_layer_status   = map_one_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_two_layer_status   = map_two_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_three_layer_status = map_three_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_four_layer_status  = map_four_layer_status + '"' + name + '","' + 'true' + '",';
    			}else if(name.search("BASELINE/") > -1){
    				map_one_layer_status   = map_one_layer_status + '"' + name + '","' + 'true' + '",';
    	    		map_two_layer_status   = map_two_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_three_layer_status = map_three_layer_status + '"' + name + '","' + 'false' + '",';
    	    		map_four_layer_status  = map_four_layer_status + '"' + name + '","' + 'false' + '",';
    			}
    		...
    		cell.set({"map_one_layer_status": map_one_layer_status});
    		cell.set({"map_two_layer_status": map_two_layer_status});
    		cell.set({"map_three_layer_status": map_three_layer_status});
    		cell.set({"map_four_layer_status": map_four_layer_status});
    		
    		return {'cell': cell, 'baseline': item.baseline_layers};
    	}
    	else{
    		return false;
    	}
    ...
    show_imagepicker_search: function(grid, e) {
    	...
    	if(this.editor_baseline_imagepicker === undefined){
    		...
    		this.editor_baseline_imagepicker.bind('baseline_success', function(){that.baselines.add(that.editor_baseline_imagepicker.baseline_layers.get_by_cell(this.cell_name))});
	...
	genarete_baseline: function(e, date_start, date_end, cell_name) {
    	if(e) e.preventDefault();
    	...
    	this.editor_baseline_imagepicker.baseline_layers.url = "/baseline_on_cell/"+date_start+"/"+date_end+"/"+cell_name+"/"
    	this.editor_baseline_imagepicker.baseline_layers.fetch({
        	                                                     success: function(){        	                	 	                                   	
